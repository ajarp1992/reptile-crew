import { useState, useEffect, useRef } from "react";

// ─── BRAND ───
const C = {
  lime: "#84CC16", limeDk: "#65A30D", limeLt: "#D9F99D",
  limeBg: "rgba(132,204,22,0.08)", limeBdr: "rgba(132,204,22,0.15)",
  bg: "#111", card: "#1c1c1c", border: "#2a2a2a", surface: "#1e1e1e",
  t1: "#f5f5f5", t2: "#999", t3: "#666",
  red: "#ef4444", redBg: "rgba(239,68,68,0.1)",
  grn: "#22c55e", grnBg: "rgba(34,197,94,0.1)",
  blu: "#3b82f6", bluBg: "rgba(59,130,246,0.1)",
};

const LOGO_SRC = "data:image/jpeg;base64,";

const uid = () => "id_" + Math.random().toString(36).slice(2, 9);
const fmtTime = (iso) => { if (!iso) return "—"; return new Date(iso).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true }); };
const fmtDate = (iso) => { if (!iso) return "—"; return new Date(iso).toLocaleDateString("en-US", { month: "short", day: "numeric" }); };
const fmtDateFull = (iso) => { if (!iso) return "—"; return new Date(iso).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" }); };
const hrs = (a, b) => { if (!a) return "0.0"; const end = b ? new Date(b) : new Date(); return ((end - new Date(a)) / 3600000).toFixed(1); };
const badge = (color, bg) => ({ display: "inline-flex", alignItems: "center", padding: "4px 10px", borderRadius: 100, fontSize: 12, fontWeight: 600, color, background: bg, letterSpacing: "0.02em" });

// ─── REAL GPS / GEOLOCATION ───
function useGPS() {
  const [status, setStatus] = useState("initializing");
  const [coords, setCoords] = useState(null);
  const [address, setAddress] = useState("");
  const [error, setError] = useState("");
  const [retryCount, setRetryCount] = useState(0);
  const watchRef = useRef(null);

  const startGPS = () => {
    if (!navigator.geolocation) {
      setStatus("unavailable");
      setError("Geolocation not supported by this browser");
      return;
    }

    // Clear any previous watch
    if (watchRef.current !== null) {
      navigator.geolocation.clearWatch(watchRef.current);
      watchRef.current = null;
    }

    setStatus("acquiring");
    setError("");

    const onSuccess = (position) => {
      const { latitude, longitude, accuracy } = position.coords;
      setCoords({ lat: latitude, lng: longitude, accuracy: Math.round(accuracy) });
      setStatus("locked");

      // Reverse geocode using free Nominatim API (OpenStreetMap)
      fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=18&addressdetails=1`)
        .then(r => r.json())
        .then(data => {
          if (data && data.display_name) {
            const a = data.address || {};
            const parts = [
              a.house_number && a.road ? `${a.house_number} ${a.road}` : a.road || "",
              a.city || a.town || a.village || "",
              a.state ? (a.state.length > 2 ? a.state.substring(0, 2).toUpperCase() : a.state) : "",
            ].filter(Boolean);
            setAddress(parts.join(", ") || data.display_name.split(",").slice(0, 3).join(","));
          }
        })
        .catch(() => {
          setAddress(`${latitude.toFixed(4)}, ${longitude.toFixed(4)}`);
        });
    };

    const onError = (err) => {
      if (err.code === 1) {
        setStatus("denied");
        setError("Location access denied");
      } else if (err.code === 2) {
        setStatus("unavailable");
        setError("Location unavailable. Check GPS settings.");
      } else {
        setStatus("timeout");
        setError("Location request timed out.");
      }
    };

    const options = { enableHighAccuracy: true, timeout: 15000, maximumAge: 60000 };
    navigator.geolocation.getCurrentPosition(onSuccess, onError, options);
    watchRef.current = navigator.geolocation.watchPosition(onSuccess, onError, { ...options, maximumAge: 30000 });
  };

  const retry = () => {
    setRetryCount(c => c + 1);
    startGPS();
  };

  useEffect(() => {
    startGPS();
    return () => {
      if (watchRef.current !== null) navigator.geolocation.clearWatch(watchRef.current);
    };
  }, []);

  return { status, coords, address, error, retry };
}

const INITIAL = {
  manager: { username: "ajarp1992@gmail.com", password: "$Aj232323", name: "AJ" },
  employees: [
    { id: "e1", name: "Marco Rivera", pin: "1234", role: "Tile Setter", phone: "(435) 555-0101", active: true },
    { id: "e2", name: "Luis Hernandez", pin: "5678", role: "Tile Setter", phone: "(435) 555-0102", active: true },
    { id: "e3", name: "Jake Thompson", pin: "9012", role: "Apprentice", phone: "(435) 555-0103", active: true },
  ],
  entries: [
    { id: "t1", eid: "e1", in: "2026-02-15T07:02:00", out: "2026-02-15T15:34:00", lat: 37.1041, lng: -113.5684, addr: "1234 Red Cliffs Dr, St. George, UT", pid: "p1" },
    { id: "t2", eid: "e2", in: "2026-02-15T06:58:00", out: "2026-02-15T16:05:00", lat: 37.0965, lng: -113.5783, addr: "890 Bluff St, St. George, UT", pid: "p1" },
    { id: "t3", eid: "e3", in: "2026-02-15T07:15:00", out: null, lat: 37.115, lng: -113.545, addr: "456 Riverside Dr, St. George, UT", pid: "p2" },
    { id: "t4", eid: "e1", in: "2026-02-14T07:00:00", out: "2026-02-14T16:00:00", lat: 37.1041, lng: -113.5684, addr: "1234 Red Cliffs Dr, St. George, UT", pid: "p1" },
    { id: "t5", eid: "e2", in: "2026-02-14T06:45:00", out: "2026-02-14T15:30:00", lat: 37.0965, lng: -113.5783, addr: "890 Bluff St, St. George, UT", pid: "p2" },
  ],
  projects: [
    { id: "p1", name: "Johnson Master Bath Remodel", addr: "1234 Red Cliffs Dr, St. George, UT", status: "active", notes: "Calacatta marble walk-in shower + heated floor", created: "2026-02-01" },
    { id: "p2", name: "Desert Springs Backsplash", addr: "456 Riverside Dr, St. George, UT", status: "active", notes: "Herringbone pattern subway tile, white grout", created: "2026-02-10" },
    { id: "p3", name: "Red Rock HOA Clubhouse", addr: "789 Canyon View, Ivins, UT", status: "upcoming", notes: "Commercial flooring — 2400 sq ft porcelain", created: "2026-02-12" },
  ],
  files: [
    { id: "f1", name: "Johnson_shower_design.pdf", pid: "p1", type: "doc", by: "AJ", date: "2026-02-01", size: "2.4 MB" },
    { id: "f2", name: "tile_layout_specs.jpg", pid: "p1", type: "photo", by: "AJ", date: "2026-02-02", size: "1.8 MB" },
    { id: "f3", name: "backsplash_inspiration.jpg", pid: "p2", type: "photo", by: "Marco Rivera", date: "2026-02-10", size: "3.1 MB" },
  ],
  msgs: [
    { id: "m1", pid: "p1", author: "AJ", text: "Tile arrived — Calacatta marble is on-site. Start demo tomorrow.", time: "2026-02-14T08:00:00" },
    { id: "m2", pid: "p1", author: "Marco Rivera", text: "Got it boss. Waterproofing done by end of day.", time: "2026-02-14T08:15:00" },
    { id: "m3", pid: "p2", author: "AJ", text: "Client wants herringbone. Reference photos in files.", time: "2026-02-13T10:00:00" },
  ],
};

// ─── ICONS ───
const I = {
  Clock: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
  Users: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
  Folder: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>,
  Pin: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
  Home: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
  Plus: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>,
  X: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
  Out: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>,
  Send: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>,
  File: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>,
  Img: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>,
  Gear: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>,
  Right: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>,
  Left: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>,
  Trash: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>,
  Msg: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>,
  Gps: (p) => <svg width={p.s||20} height={p.s||20} viewBox="0 0 24 24" fill="none" stroke={p.c||"currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><circle cx="12" cy="12" r="8"/><line x1="12" y1="2" x2="12" y2="4"/><line x1="12" y1="20" x2="12" y2="22"/><line x1="2" y1="12" x2="4" y2="12"/><line x1="20" y1="12" x2="22" y2="12"/></svg>,
};

// ─── LOGO COMPONENT ───
const Logo = ({ h = 36 }) => (
  <img src={LOGO_SRC} alt="Rep-Tile Construction" style={{ height: h, width: "auto", objectFit: "contain" }} />
);

// ─── SHARED STYLES ───
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
  @keyframes pulse2 { 0%,100% { transform: scale(1); opacity: .6; } 50% { transform: scale(1.08); opacity: .9; } }
  @keyframes ping { 0% { transform: scale(.8); opacity: 1; } 100% { transform: scale(2.5); opacity: 0; } }
  @keyframes blink { 0%,100% { opacity: 1; } 50% { opacity: .5; } }
  @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
  select option { background: #222; color: #f5f5f5; }
  input::placeholder, textarea::placeholder { color: #555; }
  ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: #333; border-radius: 4px; }
`;

const inp = (focused) => ({
  width: "100%", padding: "13px 16px", background: C.card, border: `1px solid ${focused ? C.lime : C.border}`,
  borderRadius: 12, color: C.t1, fontSize: 15, outline: "none", boxSizing: "border-box", transition: "border .2s",
  fontFamily: "inherit",
});
const btn = { padding: "13px 24px", borderRadius: 12, border: "none", cursor: "pointer", fontSize: 15, fontWeight: 600, transition: "all .2s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "inherit" };
const card = { background: C.card, borderRadius: 16, padding: 18, border: `1px solid ${C.border}` };

function Inp({ label, value, onChange, placeholder, type = "text", ...r }) {
  const [f, setF] = useState(false);
  return (
    <div style={{ marginBottom: 14 }}>
      {label && <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>{label}</div>}
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        onFocus={() => setF(true)} onBlur={() => setF(false)} style={inp(f)} {...r} />
    </div>
  );
}

function Modal({ open, onClose, title, children }) {
  if (!open) return null;
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", backdropFilter: "blur(8px)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.bg, borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 480, maxHeight: "85vh", overflowY: "auto", border: `1px solid ${C.border}`, borderBottom: "none", animation: "fadeUp .3s ease" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700 }}>{title}</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: C.t2, padding: 4 }}><I.X s={22}/></button>
        </div>
        {children}
      </div>
    </div>
  );
}

// ─── GPS STATUS CARD ───
function GPSCard({ gps }) {
  const { status, coords, address, error, retry } = gps;
  const [showHelp, setShowHelp] = useState(false);
  const isLocked = status === "locked";
  const isDenied = status === "denied";
  const isUnavailable = status === "unavailable";
  const isAcquiring = status === "acquiring" || status === "initializing";
  const needsHelp = isDenied || isUnavailable;

  // Detect device type
  const ua = typeof navigator !== "undefined" ? navigator.userAgent || "" : "";
  const isIOS = /iPhone|iPad|iPod/i.test(ua);
  const isAndroid = /Android/i.test(ua);
  const isSafari = /Safari/i.test(ua) && !/Chrome/i.test(ua);
  const isChrome = /Chrome/i.test(ua) && !/Edge/i.test(ua);

  return (
    <div style={{ ...card, marginBottom: 16, animation: "fadeUp .6s" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ position: "relative" }}>
          <div style={{ width: 44, height: 44, borderRadius: "50%", background: isLocked ? C.grnBg : needsHelp ? C.redBg : C.limeBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {isAcquiring ? (
              <div style={{ animation: "spin 1.5s linear infinite" }}><I.Gps s={20} c={C.lime}/></div>
            ) : (
              <I.Pin s={20} c={isLocked ? C.grn : needsHelp ? C.red : C.lime}/>
            )}
          </div>
          {isLocked && <div style={{ position: "absolute", inset: -4, borderRadius: "50%", border: `2px solid ${C.grn}40`, animation: "ping 2s ease-out infinite" }}/>}
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: isLocked ? C.grn : needsHelp ? C.red : C.t2 }}>
            {isLocked ? "GPS Locked" : needsHelp ? "GPS Denied" : isAcquiring ? "Acquiring GPS..." : "GPS Error"}
          </p>
          <p style={{ fontSize: 12, color: C.t3, marginTop: 2 }}>
            {isLocked ? address || "Resolving address..." : isAcquiring ? "Requesting location from device..." : error}
          </p>
          {isLocked && coords && (
            <p style={{ fontSize: 10, color: C.t3, marginTop: 3, fontFamily: "monospace" }}>
              {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)} (±{coords.accuracy}m)
            </p>
          )}
        </div>
      </div>

      {/* DENIED / UNAVAILABLE — show help + retry */}
      {needsHelp && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
          <button onClick={() => setShowHelp(!showHelp)} style={{ ...btn, background: "none", color: C.t1, padding: "0 0 10px", fontSize: 13, width: "100%", justifyContent: "space-between", borderRadius: 0 }}>
            <span>How to enable location</span>
            <span style={{ transform: showHelp ? "rotate(90deg)" : "rotate(0deg)", transition: "transform .2s", display: "flex" }}><I.Right s={16} c={C.t2}/></span>
          </button>

          {showHelp && (
            <div style={{ animation: "fadeIn .2s", marginBottom: 14 }}>
              {isIOS && isSafari && (
                <div style={{ background: C.surface, borderRadius: 12, padding: 14, marginBottom: 8 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.lime, marginBottom: 8 }}>iPhone — Safari</p>
                  <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.6 }}>
                    <p style={{ marginBottom: 4 }}>1. Open <span style={{ color: C.t1, fontWeight: 600 }}>Settings</span> app</p>
                    <p style={{ marginBottom: 4 }}>2. Scroll down, tap <span style={{ color: C.t1, fontWeight: 600 }}>Safari</span></p>
                    <p style={{ marginBottom: 4 }}>3. Tap <span style={{ color: C.t1, fontWeight: 600 }}>Location</span></p>
                    <p style={{ marginBottom: 4 }}>4. Select <span style={{ color: C.lime, fontWeight: 600 }}>Allow</span> or <span style={{ color: C.lime, fontWeight: 600 }}>Ask</span></p>
                    <p style={{ marginTop: 8, color: C.t3, fontSize: 11 }}>Also check: Settings → Privacy & Security → Location Services → make sure it's ON and Safari is set to "While Using"</p>
                  </div>
                </div>
              )}

              {isIOS && isChrome && (
                <div style={{ background: C.surface, borderRadius: 12, padding: 14, marginBottom: 8 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.lime, marginBottom: 8 }}>iPhone — Chrome</p>
                  <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.6 }}>
                    <p style={{ marginBottom: 4 }}>1. Open <span style={{ color: C.t1, fontWeight: 600 }}>Settings</span> app</p>
                    <p style={{ marginBottom: 4 }}>2. Scroll down, tap <span style={{ color: C.t1, fontWeight: 600 }}>Chrome</span></p>
                    <p style={{ marginBottom: 4 }}>3. Tap <span style={{ color: C.t1, fontWeight: 600 }}>Location</span> → set to <span style={{ color: C.lime, fontWeight: 600 }}>While Using the App</span></p>
                    <p style={{ marginTop: 8, color: C.t3, fontSize: 11 }}>Also check: Settings → Privacy & Security → Location Services is ON</p>
                  </div>
                </div>
              )}

              {isAndroid && (
                <div style={{ background: C.surface, borderRadius: 12, padding: 14, marginBottom: 8 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.lime, marginBottom: 8 }}>Android</p>
                  <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.6 }}>
                    <p style={{ marginBottom: 4 }}>1. Open <span style={{ color: C.t1, fontWeight: 600 }}>Settings</span> → <span style={{ color: C.t1, fontWeight: 600 }}>Location</span></p>
                    <p style={{ marginBottom: 4 }}>2. Make sure Location is <span style={{ color: C.lime, fontWeight: 600 }}>ON</span></p>
                    <p style={{ marginBottom: 4 }}>3. Tap <span style={{ color: C.t1, fontWeight: 600 }}>App permissions</span> → find your browser</p>
                    <p style={{ marginBottom: 4 }}>4. Set to <span style={{ color: C.lime, fontWeight: 600 }}>Allow only while using the app</span></p>
                    <p style={{ marginTop: 8, color: C.t3, fontSize: 11 }}>In Chrome: also tap the lock icon in the address bar → Permissions → Location → Allow</p>
                  </div>
                </div>
              )}

              {!isIOS && !isAndroid && (
                <div style={{ background: C.surface, borderRadius: 12, padding: 14, marginBottom: 8 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.lime, marginBottom: 8 }}>Desktop Browser</p>
                  <div style={{ fontSize: 12, color: C.t2, lineHeight: 1.6 }}>
                    <p style={{ marginBottom: 4 }}>1. Click the <span style={{ color: C.t1, fontWeight: 600 }}>lock/info icon</span> in the address bar</p>
                    <p style={{ marginBottom: 4 }}>2. Find <span style={{ color: C.t1, fontWeight: 600 }}>Location</span> permission</p>
                    <p style={{ marginBottom: 4 }}>3. Change to <span style={{ color: C.lime, fontWeight: 600 }}>Allow</span></p>
                    <p style={{ marginBottom: 4 }}>4. Refresh the page</p>
                  </div>
                </div>
              )}
            </div>
          )}

          <button onClick={retry} style={{ ...btn, background: C.lime, color: "#111", width: "100%", fontSize: 14 }}>
            <I.Gps s={16} c="#111"/> Try Again
          </button>
        </div>
      )}
    </div>
  );
}


// ═══════════════════════ LOGIN ═══════════════════════
function Login({ data, onLogin }) {
  const [mode, setMode] = useState("pick");
  const [email, setEmail] = useState("");
  const [pw, setPw] = useState("");
  const [pin, setPin] = useState("");
  const [err, setErr] = useState("");

  const doManager = () => {
    if (email.trim().toLowerCase() === data.manager.username.toLowerCase() && pw === data.manager.password) {
      onLogin({ type: "manager", user: data.manager });
    } else { setErr("Invalid email or password"); }
  };

  const doEmployee = () => {
    const e = data.employees.find(x => x.pin === pin && x.active);
    if (e) onLogin({ type: "employee", user: e });
    else { setErr("Invalid PIN"); setPin(""); }
  };

  useEffect(() => { if (pin.length === 4) doEmployee(); }, [pin]);

  return (
    <div style={{ fontFamily: "'SF Pro Display',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,sans-serif", background: C.bg, color: C.t1, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, position: "relative", overflow: "hidden" }}>
      <style>{css}</style>
      <div style={{ position: "absolute", top: -80, right: -80, width: 280, height: 280, borderRadius: "50%", background: `radial-gradient(circle,${C.lime}18,transparent 70%)`, animation: "pulse2 4s ease-in-out infinite" }}/>
      <div style={{ position: "absolute", bottom: -40, left: -40, width: 180, height: 180, borderRadius: "50%", background: `radial-gradient(circle,${C.lime}10,transparent 70%)`, animation: "pulse2 5s ease-in-out infinite 1s" }}/>

      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 340, textAlign: "center", animation: "fadeUp .5s ease" }}>
        <div style={{ marginBottom: 20, display: "flex", justifyContent: "center" }}>
          <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 16, padding: "12px 20px", border: `1px solid ${C.limeBdr}` }}>
            <Logo h={52}/>
          </div>
        </div>
        <p style={{ color: C.t2, fontSize: 13, fontWeight: 500, letterSpacing: ".15em", textTransform: "uppercase", marginBottom: 36 }}>Crew Manager</p>

        {mode === "pick" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, animation: "fadeIn .3s" }}>
            <button onClick={() => { setMode("mgr"); setErr(""); }} style={{ ...btn, background: C.lime, color: "#111", width: "100%", padding: "16px 24px", fontSize: 16 }}>
              <I.Gear s={18} c="#111"/> Manager Portal
            </button>
            <button onClick={() => { setMode("emp"); setErr(""); }} style={{ ...btn, background: C.card, color: C.t1, border: `1px solid ${C.border}`, width: "100%", padding: "16px 24px", fontSize: 16 }}>
              <I.Clock s={18}/> Employee Clock In
            </button>
          </div>
        )}

        {mode === "mgr" && (
          <div style={{ animation: "fadeIn .3s", textAlign: "left" }}>
            <Inp label="Email" value={email} onChange={setEmail} placeholder="ajarp1992@gmail.com"/>
            <Inp label="Password" value={pw} onChange={setPw} placeholder="Password" type="password"/>
            {err && <p style={{ color: C.red, fontSize: 13, marginBottom: 12, textAlign: "center" }}>{err}</p>}
            <button onClick={doManager} style={{ ...btn, background: C.lime, color: "#111", width: "100%", marginBottom: 12 }}>Sign In</button>
            <button onClick={() => { setMode("pick"); setErr(""); setEmail(""); setPw(""); }} style={{ ...btn, background: "none", color: C.t2, width: "100%" }}>Back</button>
          </div>
        )}

        {mode === "emp" && (
          <div style={{ animation: "fadeIn .3s" }}>
            <p style={{ color: C.t2, fontSize: 14, marginBottom: 20 }}>Enter your 4-digit PIN</p>
            <div style={{ display: "flex", justifyContent: "center", gap: 12, marginBottom: 20 }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ width: 52, height: 60, borderRadius: 14, background: pin[i] ? C.limeBg : C.card, border: `2px solid ${pin[i] ? C.lime : C.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 700, color: C.lime, transition: "all .2s" }}>
                  {pin[i] ? "\u2022" : ""}
                </div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, maxWidth: 240, margin: "0 auto 16px" }}>
              {[1,2,3,4,5,6,7,8,9,null,0,"del"].map((n, i) => (
                <button key={i} onClick={() => {
                  if (n === null) return;
                  if (n === "del") { setPin(pin.slice(0,-1)); setErr(""); }
                  else if (pin.length < 4) { setPin(p => p + n); setErr(""); }
                }} style={{ ...btn, background: n === null ? "transparent" : C.card, color: n === "del" ? C.t2 : C.t1, fontSize: n === "del" ? 13 : 20, fontWeight: 600, padding: "14px 0", border: n === null ? "none" : `1px solid ${C.border}`, cursor: n === null ? "default" : "pointer", borderRadius: 14 }}>
                  {n === null ? "" : n === "del" ? "DEL" : n}
                </button>
              ))}
            </div>
            {err && <p style={{ color: C.red, fontSize: 13, marginBottom: 12 }}>{err}</p>}
            <button onClick={() => { setMode("pick"); setPin(""); setErr(""); }} style={{ ...btn, background: "none", color: C.t2, width: "100%", marginTop: 4 }}>Back</button>
          </div>
        )}
      </div>
    </div>
  );
}

// ═══════════════════════ EMPLOYEE CLOCK VIEW ═══════════════════════
function EmpView({ user, data, setData, onLogout }) {
  const gps = useGPS();
  const [proj, setProj] = useState(data.projects.find(p => p.status === "active")?.id || "");
  const [now, setNow] = useState(new Date());

  useEffect(() => { const t = setInterval(() => setNow(new Date()), 1000); return () => clearInterval(t); }, []);

  const active = data.entries.find(t => t.eid === user.id && !t.out);
  const today = data.entries.filter(t => t.eid === user.id && t.in.startsWith(now.toISOString().slice(0,10)));

  const clockIn = () => {
    const e = {
      id: uid(), eid: user.id, in: new Date().toISOString(), out: null,
      lat: gps.coords?.lat || 0, lng: gps.coords?.lng || 0,
      addr: gps.address || "Location pending...", pid: proj
    };
    setData(d => ({ ...d, entries: [...d.entries, e] }));
  };
  const clockOut = () => {
    setData(d => ({ ...d, entries: d.entries.map(t => t.id === active.id ? { ...t, out: new Date().toISOString() } : t) }));
  };

  const canClock = gps.status === "locked";

  return (
    <div style={{ fontFamily: "'SF Pro Display',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,sans-serif", background: C.bg, color: C.t1, minHeight: "100vh", maxWidth: 480, margin: "0 auto" }}>
      <style>{css}</style>
      <div style={{ padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, background: "rgba(17,17,17,.92)", backdropFilter: "blur(20px)", zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Logo h={28}/>
          <div><p style={{ fontSize: 12, color: C.t2 }}>Welcome back</p><p style={{ fontSize: 15, fontWeight: 700 }}>{user.name}</p></div>
        </div>
        <button onClick={onLogout} style={{ background: "none", border: "none", cursor: "pointer", color: C.t2 }}><I.Out s={20}/></button>
      </div>

      <div style={{ padding: "20px 20px 40px" }}>
        <div style={{ textAlign: "center", marginBottom: 28, animation: "fadeUp .5s" }}>
          <p style={{ fontSize: 48, fontWeight: 200, letterSpacing: "-.02em", fontVariantNumeric: "tabular-nums" }}>
            {now.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true })}
          </p>
          <p style={{ color: C.t2, fontSize: 14, marginTop: 4 }}>{fmtDateFull(now.toISOString())}</p>
        </div>

        <GPSCard gps={gps} />

        {!active && (
          <div style={{ marginBottom: 16, animation: "fadeUp .7s" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Assign to Project</div>
            <select value={proj} onChange={e => setProj(e.target.value)} style={{ ...inp(false), appearance: "none", cursor: "pointer" }}>
              <option value="">Select Project</option>
              {data.projects.filter(p => p.status === "active").map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
        )}

        <div style={{ textAlign: "center", margin: "32px 0", animation: "fadeUp .8s" }}>
          {!active ? (
            <button onClick={clockIn} disabled={!canClock} style={{
              width: 170, height: 170, borderRadius: "50%", background: `linear-gradient(135deg,${C.lime},${C.limeDk})`,
              border: "none", cursor: canClock ? "pointer" : "default", fontSize: 17, fontWeight: 700, color: "#111",
              boxShadow: `0 0 40px ${C.lime}30, 0 8px 32px rgba(0,0,0,.4)`, opacity: canClock ? 1 : .4,
              display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "0 auto", fontFamily: "inherit",
            }}>
              <I.Clock s={30} c="#111"/><span style={{ marginTop: 8 }}>CLOCK IN</span>
            </button>
          ) : (
            <div>
              <p style={{ color: C.grn, fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Currently Clocked In</p>
              <p style={{ color: C.t2, fontSize: 13, marginBottom: 16 }}>Since {fmtTime(active.in)} — {hrs(active.in, null)} hrs</p>
              <button onClick={clockOut} style={{
                width: 170, height: 170, borderRadius: "50%", background: `linear-gradient(135deg,${C.red},#dc2626)`,
                border: "none", cursor: "pointer", fontSize: 17, fontWeight: 700, color: "#fff",
                boxShadow: `0 0 40px ${C.red}30, 0 8px 32px rgba(0,0,0,.4)`,
                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "0 auto", fontFamily: "inherit",
              }}>
                <I.Clock s={30} c="#fff"/><span style={{ marginTop: 8 }}>CLOCK OUT</span>
              </button>
            </div>
          )}
        </div>

        <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 10 }}>Today's Log</div>
        {today.length === 0 ? <p style={{ color: C.t3, fontSize: 14 }}>No entries today</p> : today.map(t => {
          const p = data.projects.find(x => x.id === t.pid);
          return (
            <div key={t.id} style={{ ...card, marginBottom: 8, padding: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <p style={{ fontSize: 14, fontWeight: 600 }}>{p?.name || "Unassigned"}</p>
                <p style={{ fontSize: 12, color: C.t2, marginTop: 2 }}>{fmtTime(t.in)} → {t.out ? fmtTime(t.out) : "Active"}</p>
              </div>
              <span style={badge(t.out ? C.t2 : C.grn, t.out ? C.surface : C.grnBg)}>{hrs(t.in, t.out)} hrs</span>
            </div>
          );
        })}
      </div>
    </div>
  );
}

// ═══════════════════════ MANAGER DASHBOARD ═══════════════════════
function Manager({ data, setData, onLogout }) {
  const [tab, setTab] = useState("home");
  const [modal, setModal] = useState(null);
  const [selProj, setSelProj] = useState(null);
  const [ef, setEf] = useState({ name: "", pin: "", role: "Tile Setter", phone: "" });
  const [pf, setPf] = useState({ name: "", addr: "", notes: "", status: "active" });
  const [ff, setFf] = useState({ name: "", type: "photo" });
  const [msg, setMsg] = useState("");

  const today = new Date().toISOString().slice(0,10);
  const todayE = data.entries.filter(t => t.in.startsWith(today));
  const live = todayE.filter(t => !t.out);
  const totalH = todayE.reduce((s, t) => s + parseFloat(hrs(t.in, t.out)), 0);
  const actProj = data.projects.filter(p => p.status === "active");
  const activeEmps = data.employees.filter(e => e.active);

  const addEmp = () => { setData(d => ({ ...d, employees: [...d.employees, { id: uid(), ...ef, active: true }] })); setEf({ name: "", pin: "", role: "Tile Setter", phone: "" }); setModal(null); };
  const rmEmp = (id) => setData(d => ({ ...d, employees: d.employees.map(e => e.id === id ? { ...e, active: false } : e) }));
  const addProj = () => { setData(d => ({ ...d, projects: [...d.projects, { id: uid(), ...pf, created: today }] })); setPf({ name: "", addr: "", notes: "", status: "active" }); setModal(null); };
  const sendMsg = () => { if (!msg.trim() || !selProj) return; setData(d => ({ ...d, msgs: [...d.msgs, { id: uid(), pid: selProj.id, author: "AJ", text: msg.trim(), time: new Date().toISOString() }] })); setMsg(""); };
  const addFile = () => { if (!selProj) return; setData(d => ({ ...d, files: [...d.files, { id: uid(), pid: selProj.id, ...ff, by: "AJ", date: today, size: `${(Math.random()*5+.5).toFixed(1)} MB` }] })); setFf({ name: "", type: "photo" }); setModal(null); };

  const navItems = [
    { k: "home", icon: I.Home, label: "Home" },
    { k: "time", icon: I.Clock, label: "Time" },
    { k: "team", icon: I.Users, label: "Team" },
    { k: "projects", icon: I.Folder, label: "Projects" },
  ];

  const renderHome = () => (
    <div style={{ padding: "16px 20px 120px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[{ v: live.length, l: "Clocked In", c: C.lime }, { v: totalH.toFixed(1), l: "Hours Today", c: C.t1 }, { v: activeEmps.length, l: "Crew", c: C.t1 }, { v: actProj.length, l: "Active Jobs", c: C.t1 }].map((s, i) => (
          <div key={i} style={{ ...card, textAlign: "center", animation: `fadeUp ${.3 + i * .08}s ease` }}>
            <p style={{ fontSize: 30, fontWeight: 200, color: s.c }}>{s.v}</p>
            <p style={{ fontSize: 10, color: C.t2, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", marginTop: 4 }}>{s.l}</p>
          </div>
        ))}
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em" }}>Live Crew Status</div>
        <span style={{ ...badge(C.grn, C.grnBg), animation: "blink 2s infinite" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.grn, marginRight: 6, display: "inline-block" }}/>LIVE
        </span>
      </div>

      {activeEmps.map((emp, i) => {
        const entry = live.find(t => t.eid === emp.id);
        const proj = entry ? data.projects.find(p => p.id === entry.pid) : null;
        return (
          <div key={emp.id} style={{ ...card, marginBottom: 8, padding: 14, animation: `fadeUp ${.5 + i * .08}s ease` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <div style={{ width: 38, height: 38, borderRadius: 10, background: entry ? C.grnBg : C.surface, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: entry ? C.grn : C.t3 }}>
                  {emp.name.split(" ").map(n => n[0]).join("")}
                </div>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600 }}>{emp.name}</p>
                  <p style={{ fontSize: 12, color: C.t2, marginTop: 1 }}>{emp.role}</p>
                </div>
              </div>
              <span style={badge(entry ? C.grn : C.t3, entry ? C.grnBg : C.surface)}>{entry ? "On Site" : "Off"}</span>
            </div>
            {entry && (
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: C.t2 }}><I.Pin s={11} c={C.grn}/>{entry.addr}</div>
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5 }}>
                  <span style={{ fontSize: 12, color: C.t2 }}>{proj?.name || "Unassigned"}</span>
                  <span style={{ fontSize: 12, color: C.lime, fontWeight: 600 }}>{hrs(entry.in, null)} hrs</span>
                </div>
              </div>
            )}
          </div>
        );
      })}
    </div>
  );

  const renderTime = () => {
    const grouped = {};
    [...data.entries].reverse().forEach(t => {
      const d = t.in.slice(0,10);
      if (!grouped[d]) grouped[d] = [];
      grouped[d].push(t);
    });
    return (
      <div style={{ padding: "16px 20px 120px" }}>
        {Object.entries(grouped).slice(0,7).map(([day, ents]) => (
          <div key={day} style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>{fmtDateFull(day + "T00:00:00")}</div>
            {ents.map(t => {
              const emp = data.employees.find(e => e.id === t.eid);
              const proj = data.projects.find(p => p.id === t.pid);
              return (
                <div key={t.id} style={{ ...card, marginBottom: 8, padding: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600 }}>{emp?.name || "?"}</p>
                      <p style={{ fontSize: 12, color: C.t2, marginTop: 2 }}>{proj?.name || "Unassigned"}</p>
                    </div>
                    <span style={badge(t.out ? C.t1 : C.grn, t.out ? C.surface : C.grnBg)}>{hrs(t.in, t.out)} hrs</span>
                  </div>
                  <div style={{ marginTop: 8, display: "flex", justifyContent: "space-between", fontSize: 12, color: C.t3 }}>
                    <span>{fmtTime(t.in)} → {t.out ? fmtTime(t.out) : "Active"}</span>
                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><I.Pin s={10}/>{t.addr?.split(",")[0]}</span>
                  </div>
                </div>
              );
            })}
          </div>
        ))}
      </div>
    );
  };

  const renderTeam = () => (
    <div style={{ padding: "16px 20px 120px" }}>
      <button onClick={() => setModal("emp")} style={{ ...btn, background: C.lime, color: "#111", width: "100%", marginBottom: 16 }}><I.Plus s={18} c="#111"/> Add Employee</button>
      {activeEmps.map(emp => (
        <div key={emp.id} style={{ ...card, marginBottom: 8, padding: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <div style={{ width: 42, height: 42, borderRadius: 12, background: C.limeBg, border: `1px solid ${C.limeBdr}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: C.lime }}>
                {emp.name.split(" ").map(n => n[0]).join("")}
              </div>
              <div>
                <p style={{ fontSize: 15, fontWeight: 600 }}>{emp.name}</p>
                <p style={{ fontSize: 12, color: C.t2, marginTop: 1 }}>{emp.role}</p>
              </div>
            </div>
            <button onClick={() => rmEmp(emp.id)} style={{ background: "none", border: "none", cursor: "pointer", color: C.t3, padding: 4 }}><I.Trash s={16}/></button>
          </div>
          <div style={{ height: 1, background: C.border, margin: "12px 0" }}/>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.t2 }}>
            <span>PIN: {emp.pin}</span><span>{emp.phone}</span>
          </div>
        </div>
      ))}
    </div>
  );

  const renderProjects = () => {
    if (selProj) {
      const pFiles = data.files.filter(f => f.pid === selProj.id);
      const pMsgs = data.msgs.filter(m => m.pid === selProj.id);
      const pH = data.entries.filter(t => t.pid === selProj.id).reduce((s, t) => s + parseFloat(hrs(t.in, t.out)), 0);
      return (
        <div style={{ padding: "16px 20px 120px" }}>
          <button onClick={() => setSelProj(null)} style={{ ...btn, background: "none", color: C.t2, padding: "8px 0", marginBottom: 16, fontSize: 13, justifyContent: "flex-start" }}><I.Left s={16}/> All Projects</button>

          <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>{selProj.name}</h2>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 13, color: C.t2, marginBottom: 16 }}><I.Pin s={13}/>{selProj.addr}</div>

          <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
            {[{ v: pH.toFixed(1), l: "Hours" }, { v: pFiles.length, l: "Files" }, { v: pMsgs.length, l: "Messages" }].map((s, i) => (
              <div key={i} style={{ ...card, flex: 1, textAlign: "center", padding: 14 }}>
                <p style={{ fontSize: 22, fontWeight: 200, color: C.lime }}>{s.v}</p>
                <p style={{ fontSize: 9, color: C.t2, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", marginTop: 2 }}>{s.l}</p>
              </div>
            ))}
          </div>

          {selProj.notes && <div style={{ ...card, marginBottom: 16, fontSize: 13, color: C.t2 }}>{selProj.notes}</div>}

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em" }}>Files</div>
            <button onClick={() => setModal("file")} style={{ background: "none", border: "none", cursor: "pointer", color: C.lime, fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}>+ Add</button>
          </div>
          {pFiles.length === 0 ? <p style={{ color: C.t3, fontSize: 13, marginBottom: 16 }}>No files yet</p> : pFiles.map(f => (
            <div key={f.id} style={{ ...card, marginBottom: 6, padding: 12, display: "flex", alignItems: "center", gap: 10 }}>
              {f.type === "photo" ? <I.Img s={16} c={C.lime}/> : <I.File s={16} c={C.blu}/>}
              <div style={{ flex: 1 }}><p style={{ fontSize: 13, fontWeight: 600 }}>{f.name}</p><p style={{ fontSize: 11, color: C.t3 }}>{f.by} — {f.size}</p></div>
            </div>
          ))}

          <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 10, marginTop: 20 }}>Messages</div>
          {pMsgs.map(m => (
            <div key={m.id} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ fontWeight: 700, color: m.author === "AJ" ? C.lime : C.t1 }}>{m.author}</span>
                <span style={{ color: C.t3 }}>{fmtTime(m.time)}</span>
              </div>
              <p style={{ fontSize: 13, color: C.t2, marginTop: 2 }}>{m.text}</p>
            </div>
          ))}
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <input value={msg} onChange={e => setMsg(e.target.value)} placeholder="Send an update..." onKeyDown={e => e.key === "Enter" && sendMsg()} style={{ ...inp(false), flex: 1 }}/>
            <button onClick={sendMsg} style={{ ...btn, background: C.lime, color: "#111", padding: "12px 16px" }}><I.Send s={16} c="#111"/></button>
          </div>
        </div>
      );
    }

    return (
      <div style={{ padding: "16px 20px 120px" }}>
        <button onClick={() => setModal("proj")} style={{ ...btn, background: C.lime, color: "#111", width: "100%", marginBottom: 16 }}><I.Plus s={18} c="#111"/> New Project</button>
        {["active", "upcoming", "completed"].map(st => {
          const filtered = data.projects.filter(p => p.status === st);
          if (!filtered.length) return null;
          return (
            <div key={st}>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8, marginTop: 8 }}>{st}</div>
              {filtered.map(proj => {
                const pH = data.entries.filter(t => t.pid === proj.id).reduce((s, t) => s + parseFloat(hrs(t.in, t.out)), 0);
                return (
                  <div key={proj.id} onClick={() => setSelProj(proj)} style={{ ...card, marginBottom: 8, padding: 14, cursor: "pointer" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: 15, fontWeight: 600 }}>{proj.name}</p>
                        <p style={{ fontSize: 12, color: C.t2, display: "flex", alignItems: "center", gap: 4, marginTop: 4 }}><I.Pin s={11}/>{proj.addr}</p>
                      </div>
                      <I.Right s={18} c={C.t3}/>
                    </div>
                    <div style={{ marginTop: 10, display: "flex", gap: 14, fontSize: 12, color: C.t3 }}>
                      <span style={{ display: "flex", alignItems: "center", gap: 4 }}><I.Clock s={11}/>{pH.toFixed(1)} hrs</span>
                      <span style={{ display: "flex", alignItems: "center", gap: 4 }}><I.File s={11}/>{data.files.filter(f => f.pid === proj.id).length}</span>
                      <span style={{ display: "flex", alignItems: "center", gap: 4 }}><I.Msg s={11}/>{data.msgs.filter(m => m.pid === proj.id).length}</span>
                    </div>
                  </div>
                );
              })}
            </div>
          );
        })}
      </div>
    );
  };

  return (
    <div style={{ fontFamily: "'SF Pro Display',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,sans-serif", background: C.bg, color: C.t1, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      <style>{css}</style>

      <div style={{ padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "rgba(17,17,17,.92)", backdropFilter: "blur(20px)", zIndex: 50, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Logo h={30}/>
        </div>
        <button onClick={onLogout} style={{ background: "none", border: "none", cursor: "pointer", color: C.t2, display: "flex", alignItems: "center", gap: 5, fontSize: 13, fontFamily: "inherit" }}><I.Out s={16}/></button>
      </div>

      {tab === "home" && renderHome()}
      {tab === "time" && renderTime()}
      {tab === "team" && renderTeam()}
      {tab === "projects" && renderProjects()}

      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "rgba(17,17,17,.95)", backdropFilter: "blur(20px)", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-around", padding: "8px 0 24px", zIndex: 100 }}>
        {navItems.map(n => (
          <button key={n.k} onClick={() => { setTab(n.k); setSelProj(null); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", color: tab === n.k ? C.lime : C.t3, fontSize: 10, fontWeight: 600, letterSpacing: ".05em", padding: "4px 12px", background: "none", border: "none", fontFamily: "inherit", transition: "color .2s" }}>
            <n.icon s={22} c={tab === n.k ? C.lime : C.t3}/><span>{n.label}</span>
          </button>
        ))}
      </div>

      {/* MODALS */}
      <Modal open={modal === "emp"} onClose={() => setModal(null)} title="Add Employee">
        <Inp label="Full Name" value={ef.name} onChange={v => setEf({...ef, name: v})} placeholder="e.g. Carlos Mendez"/>
        <Inp label="4-Digit PIN" value={ef.pin} onChange={v => setEf({...ef, pin: v.slice(0,4)})} placeholder="e.g. 3456" maxLength={4}/>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Role</div>
          <select value={ef.role} onChange={e => setEf({...ef, role: e.target.value})} style={{ ...inp(false), appearance: "none", cursor: "pointer" }}>
            <option>Tile Setter</option><option>Apprentice</option><option>Flooring Installer</option><option>Foreman</option>
          </select>
        </div>
        <Inp label="Phone" value={ef.phone} onChange={v => setEf({...ef, phone: v})} placeholder="(435) 555-0000"/>
        <button onClick={addEmp} disabled={!ef.name || !ef.pin} style={{ ...btn, background: C.lime, color: "#111", width: "100%", opacity: ef.name && ef.pin ? 1 : .4, marginTop: 6 }}><I.Plus s={18} c="#111"/> Add Employee</button>
      </Modal>

      <Modal open={modal === "proj"} onClose={() => setModal(null)} title="New Project">
        <Inp label="Project Name" value={pf.name} onChange={v => setPf({...pf, name: v})} placeholder="e.g. Smith Kitchen Backsplash"/>
        <Inp label="Address" value={pf.addr} onChange={v => setPf({...pf, addr: v})} placeholder="123 Main St, St. George, UT"/>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Status</div>
          <select value={pf.status} onChange={e => setPf({...pf, status: e.target.value})} style={{ ...inp(false), appearance: "none", cursor: "pointer" }}>
            <option value="active">Active</option><option value="upcoming">Upcoming</option><option value="completed">Completed</option>
          </select>
        </div>
        <Inp label="Notes" value={pf.notes} onChange={v => setPf({...pf, notes: v})} placeholder="Tile type, special instructions..."/>
        <button onClick={addProj} disabled={!pf.name} style={{ ...btn, background: C.lime, color: "#111", width: "100%", opacity: pf.name ? 1 : .4, marginTop: 6 }}><I.Plus s={18} c="#111"/> Create Project</button>
      </Modal>

      <Modal open={modal === "file"} onClose={() => setModal(null)} title="Add File">
        <Inp label="File Name" value={ff.name} onChange={v => setFf({...ff, name: v})} placeholder="e.g. shower_progress.jpg"/>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.t2, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Type</div>
          <div style={{ display: "flex", gap: 10 }}>
            {["photo", "doc"].map(t => (
              <button key={t} onClick={() => setFf({...ff, type: t})} style={{ ...btn, flex: 1, padding: "12px 16px", background: ff.type === t ? C.limeBg : C.card, color: ff.type === t ? C.lime : C.t2, border: `1px solid ${ff.type === t ? C.lime : C.border}`, fontSize: 13 }}>
                {t === "photo" ? <I.Img s={16}/> : <I.File s={16}/>}{t === "photo" ? "Photo" : "Document"}
              </button>
            ))}
          </div>
        </div>
        <button onClick={addFile} disabled={!ff.name} style={{ ...btn, background: C.lime, color: "#111", width: "100%", opacity: ff.name ? 1 : .4, marginTop: 6 }}><I.Plus s={18} c="#111"/> Add File</button>
      </Modal>
    </div>
  );
}

// ═══════════════════════ MAIN APP ═══════════════════════
export default function App() {
  const [data, setData] = useState(INITIAL);
  const [auth, setAuth] = useState(null);

  if (!auth) return <Login data={data} onLogin={setAuth}/>;
  if (auth.type === "employee") return <EmpView user={auth.user} data={data} setData={setData} onLogout={() => setAuth(null)}/>;
  return <Manager data={data} setData={setData} onLogout={() => setAuth(null)}/>;
}
